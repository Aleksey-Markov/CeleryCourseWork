    ...
    def __call__(self, *args, **kwargs):
        ...
        if tmp_val1 and tmp_val2:
            raise ValidationError(
                "Нужно выбрать ЛИБО вознаграждение ЛИБО полезную привычку."
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if tmp_val:
            if not tmp_val.is_enjoyable:
                raise ValidationError(
                    "Связанная привычка может быть только для приятной привычки."
                )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if tmp_val:
            if tmp_val > timedelta(seconds=120):
                raise ValidationError(
                    "Продолжительность не может быть больше 2-х минут."
                )
